...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, label, inst, image:list, feat, infer=False):
        # Encode Inputs
        input_label, inst_map, real_image, feat_map = self.encode_input(label, inst, image[0], feat)  

        # Fake Generation
        min_c = -0.6
        max_c = 0.6
        inter=(max_c-min_c)/(self.opt.domain_num-1)
        N,C,H,W = input_label.size()
        ...
        loss_G_GAN_Feat = 0
        loss_G_VGG = 0
        loss_G_Grad = loss_G_ssim = loss_G_tv = 0
        fake_images=[]
        for i in range(self.opt.domain_num):
            real_image = Variable(image[i].data.cuda())
            tensor_c=(torch.ones(N,1,H,W)*(min_c + i*inter)).cuda()
            input_concat=torch.cat((input_label, tensor_c), dim=1)
        ...
        return [ self.loss_filter(loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake, loss_G_Grad, loss_G_ssim, loss_G_tv), None if not infer else fake_images ]
